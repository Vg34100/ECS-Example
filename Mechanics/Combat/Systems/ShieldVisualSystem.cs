...
{
    /// <summary>
    /// Positions a shield visual in front of the player while blocking.
    /// </summary>
    public class ShieldVisualSystem
    {
        public void Update(World world, float deltaTime)
        {
            foreach (var entity in world.Query<ShieldVisualComponent, PositionComponent, ShapeComponent>())
            {
                var visual = world.GetComponent<ShieldVisualComponent>(entity);
                var target = world.GetEntities().FirstOrDefault(e => e.Id == visual.TargetEntityId);
                if (target == null)
                    continue;

                bool isBlocking = world.TryGetComponent<BlockingComponent>(target, out var block) && block.IsBlocking;
                if (!isBlocking)
                {
                    ...
                    if (world.TryGetComponent<ShapeComponent>(entity, out var shapeOff))
                    {
