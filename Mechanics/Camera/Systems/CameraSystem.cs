...
namespace ECS_Base.Mechanics.Camera.Systems
{
    public class CameraSystem
    {
        ...
        public void Update(World world, float deltaTime)
        {
            ...
            if (!world.TryGetComponent<CameraComponent>(cameraEntity, out var camera))
                return;

            // Find the target entity (player)
            var targetEntity = world.GetEntities()
                .FirstOrDefault(e => world.TryGetComponent<CameraTargetComponent>(e, out var target) && target.IsActive);

            if (targetEntity != null && world.TryGetComponent<PositionComponent>(targetEntity, out var targetPos))
            {
                ...
                if (distance > camera.DampeningThreshold)
                {
                    ...
                    Vector2 desiredVelocity = difference * (1.0f - camera.LagFactor) * 60.0f;
